...
function draw(err, data) {
  ...
  const color = d3.scale.ordinal()
      .range(["hsl(200,100%,10%)","hsl(200,100%,20%)", "hsl(200,100%,30%)", "hsl(200,100%,40%)", "hsl(200,100%,50%)", "hsl(200,100%,60%)", "hsl(200,100%,70%)"]);

  selectedWeek = d3.min( data.map( (d) => ( d["Week"] ) ) );

  const typesOfminutes = d3.keys(data[0]).filter((key) => (
    key !== "Datum" &&
    key !== "Dag" &&
    key !== "Week" &&
    key !== "Aantal broodjes" &&
    key !== "Aantal glazen water" &&
    key !== "Aantal koppen koffie" &&
    key !== "Blijheid" &&
    key !== "Slaap kwaliteit" &&
    key !== "Plaatsnamen" &&
    key !== "Euro uitgegeven" &&
    key !== "Aantal werkafspraken" &&
    key !== "Slaap minuten"
  ));
  ...
  data.forEach((d, i) => {
    ...
    for (let k = 0; k < typesOfminutes.length; k++) {
      ...
      d.zeroMinutes[k] === 0 ? d[type] = 1 : false;
    }
  });
  ...
  */
  function update(data) {
    ...
    .enter().append('rect')
      ...
      .attr('y', (d) => ( y0(d.value) ))
      .attr('height', (d) => ( m.innerHeight() - y0(d.value) ))
